...
def test_dropbox_oauth_finish_handles_badstateexception(client):
    ...
    query_string = {'state': 'sojhqf2nGunEIxI',
                    'code': 'pQbO_7SMV2AAAAAAAAAAL'}
    response = client.get('/dropbox/finish', query_string=query_string)
...
def test_dropbox_oauth_finish_handles_csrfexception(client):
    """
    This test triggers a CsrfException by setting different csrf token
    and state token
    """
    with client.session_transaction() as sess:
        sess['dropbox-auth-csrf-token'] = 'notTheStateInTheQueryString'
    query_string = {'state': 'somethingDifferent',
                    'code': 'pQbO_7SMV2AAAAAAAAAAL'}
...
def test_dropbox_oauth_handles_app_notapprovedexception(client, monkeypatch):
    def mock_dropbox_finish(*args):
